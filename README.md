# 📅 Petshop Appointment Service (`petshop-appointment-service`)

Este repositório contém o **Microsserviço de Agendamentos** da arquitetura distribuída do Petshop. Ele representa um dos lados de **Escrita (Command)** no padrão CQRS (Command Query Responsibility Segregation).

---

## 🏗️ Papel e Funcionalidade no Ecossistema

O `appointment-service` é o motor de agendamentos e validações de calendário da aplicação:
1. **Domínio de Agendamento**: Cria e cancela agendamentos para serviços (por exemplo, Banho e Tosa).
2. **Validações Rígidas de Negócios**:
   * **Prevenção de Sobreposição (Overlap)**: Garante que não haja concorrência de horários de forma global na loja.
   * **Unicidade de Pet**: Impede que o mesmo animal possua mais de um agendamento futuro ativo simultaneamente.
3. **Persistência Relacional**: Utiliza **PostgreSQL** para persistir as agendas físicas.
   * **Ambiente DEV**: Grava na base através do schema `dev`.
   * **Ambiente HOMOL/PROD**: Grava na base através do schema `homol`.
4. **Mensageria Assíncrona (Event Sourcing / CQRS Sync)**: Após as validações de negócio e persistência na base relacional, o microsserviço publica eventos de negócio reativos na fila do **RabbitMQ**:
   * Evento: `AppointmentScheduledEvent` (com os detalhes do agendamento criado).
   * Evento: `AppointmentCancelledEvent` (com o ID do agendamento que foi removido).
   * Esses eventos serão ouvidos pelo `query-service` para manter as views atualizadas de forma síncrona/reativa.
...
* **Hibernate ORM com Panache** (Simplificação de persistência estruturada)
* **JDBC Driver - PostgreSQL** (Conectividade relacional robusta)
* **SmallRye Reactive Messaging - RabbitMQ Connector** (Mensageria assíncrona orientada a eventos)
...
* **Porta local padrão**: `8083`
* **Painel Dev UI do Quarkus**: `http://localhost:8083/q/dev/`
...
* **Endpoint de Métricas**: `GET http://localhost:8083/q/metrics`
...
* **Swagger UI (Interface Visual)**: `http://localhost:8082/q/swagger-ui/`
  * No Render (DEV): [https://petshop-appointment-service-dev.onrender.com/q/swagger-ui/](https://petshop-appointment-service-dev.onrender.com/q/swagger-ui/)
* **OpenAPI Spec (Esquema JSON)**: `http://localhost:8082/q/openapi`
  * No Render (DEV): [https://petshop-appointment-service-dev.onrender.com/q/openapi](https://petshop-appointment-service-dev.onrender.com/q/openapi)
...
1. **Continuous Integration (`ci.yml`)**:
   ...
   * Envia análises de qualidade estática para o **SonarCloud** (Project Key: `ocsane-figueira_petshop-appointment-service`).
   * Para pushes aprovados em `main`, constrói a imagem Docker oficial multi-stage e envia para o Docker Hub com tags SHA e `main` (`ocsane/petshop-appointment-service`).
